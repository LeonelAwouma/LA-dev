...
export function MoveHistory() {
  ...
  return (
    <SidebarGroup>
      <SidebarGroupLabel>Move History</SidebarGroupLabel>
      <SidebarGroupContent>
        <ScrollArea className="h-48 w-full rounded-md border" ref={scrollAreaRef}>
          {history.length === 0 ? (
            <div className="p-4 text-center text-sm text-muted-foreground">No moves yet.</div>
          ) : (
            <div className="p-2 text-sm font-mono">
              <ol className="grid grid-cols-[auto_1fr_1fr] gap-x-2 gap-y-1">
                {Array.from({ length: Math.ceil(history.length / 2) }).map((_, i) => {
                  ...
                  return (
                    <li key={i} className="contents">
                      ...
                      <span className="px-1">{whiteMove?.san || '...'}</span>
                      <span className="px-1">{blackMove?.san || '...'}</span>
                    </li>
                  );
                })}
              </ol>
