...
const SuggestMoveWithAnalysisOutputSchema = z.object({
  move: z.string().describe('The suggested move in UCI notation (e.g., "e2e4").'),
  analysis: z
    .string()
...
Based on the current board state and difficulty, suggest the single best move and explain your reasoning in a concise manner.

Current Board State (FEN): {{{fen}}}
AI Difficulty (Elo): {{{difficulty}}}

Respond with a JSON object containing the suggested move in UCI notation and a brief explanation of your reasoning.
`,
});
